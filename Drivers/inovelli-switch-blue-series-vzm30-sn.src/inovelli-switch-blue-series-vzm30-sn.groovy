def getDriverDate() { return "2025-07-18" }	// **** DATE OF THE DEVICE DRIVER
...
metadata {
    definition (name: "Inovelli Switch Blue Series VZM30-SN", namespace: "InovelliUSA", author: "E.Maycock/M.Amber", filename: "Inovelli-zigbee-switch", importUrl:"https://raw.githubusercontent.com/InovelliUSA/Hubitat/refs/heads/master/Drivers/inovelli-switch-blue-series-vzm30-sn.src/inovelli-switch-blue-series-vzm30-sn.groovy" ) 
	{
        ...
        */
        ...
        fingerprint profileId:"0104", endpointId:"01", inClusters:"0000,0003,0004,0005,0006,0008,0702,0B04,0B05,FC31,FC57", outClusters:"0019",                model:"VZM30-SN", manufacturer:"Inovelli"
        fingerprint profileId:"0104", endpointId:"02", inClusters:"0000,0003",                                              outClusters:"0003,0006,0008,FC31", model:"VZM30-SN", manufacturer:"Inovelli"
    }
...
//	cmds += ["zdo bind ${device.deviceNetworkId} 0x01 0x01 0x0202 {${device.zigbeeId}} {}"] //Fan Control Cluster
    ...
    cmds += ["zdo bind ${device.deviceNetworkId} 0x04 0x01 0x0402 {${device.zigbeeId}} {}"] //Temperature Measurement - to get temperature reports
	cmds += ["zdo bind ${device.deviceNetworkId} 0x04 0x01 0x0405 {${device.zigbeeId}} {}"] //Humidity Measurement - to get humidity reports
        ...
        case 0x0000:    //BASIC CLUSTER
            ...
            switch (attrInt) {
                ...
                case 0x0005:
					...
					if (valueStr=="unknown" || valueStr==null) valueStr = "VZM31-SN"
            ...
            switch (attrInt) {
                case 0x0000:
                    if (descMap.command == "0A"){
                        float temperature = Integer.parseInt(descMap['value'],16)/100
                        temperature = (location.temperatureScale == "F") ? ((temperature * 1.8) + 32) : temperature
                        temperature = temperature.round(2)
                        if (infoEnable) log.info "${device.displayName} Temperature=${temperature}"
                        sendEvent(name:"temperature",value:temperature ,unit: location.temperatureScale)
                    } else log.warn "${device.displayName} " + fireBrick("${clusterName} Unknown Command:$descMap.command ") //+ descMap		  													  
                    break
                default:
                    log.warn "${device.displayName} " + fireBrick("${clusterName} Unknown Attribute:$attrInt ") //+ descMap
            ...
            switch (attrInt) {
                case 0x0000:
                    if (descMap.command == "0A"){
                        valueInt = Integer.parseInt(descMap['value'],16)
                        float humidity
                        humidity = valueInt/100
                        if (infoEnable) log.info "${device.displayName} Humidity=${humidity}"
