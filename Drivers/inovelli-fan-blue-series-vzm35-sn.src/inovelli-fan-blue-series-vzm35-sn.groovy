...
* 2023-05-01(MA) updates for MG24 v0.01 firmware; misc. minor bug fixes
...
* 2023-07-01(MA) removed "beta" designation for public release (V0.02)
...
metadata {
    definition (name: "Inovelli Fan Switch Blue Series VZM35-SN", namespace: "InovelliUSA", author: "E.Maycock/M.Amber", filename: "Inovelli-zigbee-fan-switch", importUrl:"https://raw.githubusercontent.com/InovelliUSA/Hubitat/master/Drivers/inovelli-fan-blue-series-vzm35-sn.src/inovelli-fan-blue-series-vzm35-sn.groovy") 
	{
        ...
        attribute "ledEffect", "String"			//LED effect that was last requested (may have timed-out and not necessarily displaying currently)
...
//      fingerprint profileId:"0104", endpointId:"01", inClusters:"0000,0003,0004,0005,0006,0008,0B05,FC57,FC31", outClusters:"0003,0019",           model:"VZM35-SN",      manufacturer:"Inovelli", deviceTypeId:"193"
//      fingerprint profileId:"0104", endpointId:"02", inClusters:"0000,0003",                                    outClusters:"0003,0006,0008,FC31", model:"VZM35-SN",      manufacturer:"Inovelli"
//      fingerprint profileId:"0104", endpointId:"01", inClusters:"0000,0003,0004,0005,0006,0008,FC31",           outClusters:"0003,0019",           model:"VZM35-SN",      manufacturer:"Inovelli"
    }
...
@Field static Map configParams = [
    ...
    parameter023 : [ //implemented in firmware for the fan, emulated in this driver for 2-in-1 Dimmer
        number: 23,
        name: "Quick Start",
        description: "Duration (in seconds) of full power output when the fan goes from OFF to ON (0=disabled)",
        range: "0..10",
        default: 0,
        size: 8,
        type: "number",
...
def clusterLookup(cluster) {
	if (cluster==null) return "un-named cluster"
	else {
...
def initialize() {    //CALLED DURING HUB BOOTUP IF "INITIALIZE" CAPABILITY IS DECLARED IN METADATA SECTION
    ...
    if (infoEnable) log.info "${device.displayName} clearing state variables..."
    ...
    def cmds = []
    if (infoEnable) log.info "${device.displayName} clearing LED notifications..."
		...
		switch (option) {
			case "":
				...
				&& (!getReadOnlyParams().contains(i))) {	//AND  this is not a read-only parameter
					cmds += setParameter(i, newValue, 150)	//THEN set the new value
			...
			case "Default":
				...
				break
			default: 
				if (traceEnable||debugEnable) log.error "${device.displayName} Unknonwn option 'updated($option)'"
				break
		}
