...
* 2023-05-01(MA) misc. minor bug fixes; updates for MG24 v0.01 firmware
...
* 2023-04-09(MA) added range/error checking to group binding; enhanced cluster name reporting
...
* 2022-12-26(MA) allow 5-speed in Smart Fan Mode; add Speed Up/Down; add Param263 (fw 1.08+)
* 2022-12-21(MA) add Smart Fan Mode (fw 1.08+)
...
* 2022-11-03(MA) addeded param 262; added additional LED effects rising,falling,fast/slow, etc.
* 2022-11-02(EM) added warning for firmware update and requirement for double click.
* 2022-10-07(MA) add new Led effects for V1.07 (rising/falling, etc.)
* 2022-09-16(MA) fix typo in QuickStart range definition
* 2022-09-15(MA) make QuickStart option available in multi-speed mode
* 2022-09-10(MA) fix parameter descriptions that accidentally got set back to "light" instead of "fan"
...
* 2022-06-08(MA) lots of cleanup and minor bug fixes; add some code to detect model and more merges with VZM31
* 2022-06-07(MA) add html enhancements to input boxes
* 2022-06-06(MA) more updates for v4 firmware
...
* 2022-05-30(MA) updates for v4 firmware
* 2022-04-11(MA) accept med-low and med-high setting but just convert to low and med
* 2022-04-10(MA) if in on/off mode, convert 'cycle speed' to 'toggle on/off'
* 2022-03-26(MA) merge with VZM31-SN 2022-03-26 updates.  (this is still behind on firmware changes)
...
* 2022-03-10(MA) Change params 13,14,15 to drop-downs.  implemented CycleSpeed and SetSpeed commands
* 2022-03-09(MA) Merge with changes in the 2022-03-09 VZM31-SN 2-in-1 Dimmer driver 
* 2022-03-07(MA) Merge with changes in the 2022-03-07 VZM31-SN 2-in-1 Dimmer driver         
* 2022-03-05(MA) Add FanControl capability
* 2022-03-02(MA) Updates based off 2022-03-02 VZM31-SN driver    
* 2022-02-28(MA) Initial release - based off 2022-02-25 VZM31-SN driver
*
*/
...
metadata {
    definition (name: "Inovelli Fan Switch Blue Series VZM35-SN", namespace: "InovelliUSA", author: "E.Maycock/M.Amber", filename: "Inovelli-zigbee-fan-switch", importUrl:"https://raw.githubusercontent.com/InovelliUSA/Hubitat/master/Drivers/inovelli-fan-blue-series-vzm35-sn.src/inovelli-fan-blue-series-vzm35-sn.groovy") 
	{
...
def ledEffectAll(effect=255, color=0, level=100, duration=60) {
    ...
    switch (effect){
		...
		default:	effectName = "Undefined Effect #$effect"; break
	}
        ...
        case 0x0202:    //FAN CONTROL CLUSTER
            ...
            switch (attrInt) {
                case 0x0000:	//Fan Mode
					def fanMode = "Auto"	//default value per ZCL specification
					valueInt = Integer.parseInt(descMap['value'],16)
					switch (valueInt) {
						case 0: fanMode="Off";		break
						case 1: fanMode="Low";		break
						case 2: fanMode="Medium";	break
						case 3: fanMode="High";		break
						case 4: fanMode="On";		break
						case 5: fanMode="Auto";		break
						case 6: fanMode="Smart";	break
					}
					if (infoEnable) log.info "${device.displayName} Fan Mode=$fanMode"
                    break
				case 0x0001:	//Fan Mode Sequence
					def fanSeq = "Low/Med/High/Auto"	//default value per ZCL specification
					valueInt = Integer.parseInt(descMap['value'],16)
					switch (valueInt) {
						case 0: fanSeq="Low/Med/High";		break
						case 1: fanSeq="Low/High";			break
						case 2: fanSeq="Low/Med/High/Auto";	break
						case 3: fanSeq="Low/High/Auto";		break
						case 4: fanSeq="On/Auto";			break
					}
					if (infoEnable) log.info "${device.displayName} Fan Sequence=$fanSeq"
                    break
                default:
                    log.warn "${device.displayName} "+fireBrick("${clusterName} Unknown Attribute:$attrInt ") //+ descMap
                    break
...
//	cmds += zigbee.readAttribute(0x0702, 0x0000, [:], shortDelay)	//SIMPLE_METERING Energy Report	//Fan does not support power/energy reports
...
//  cmds += zigbee.readAttribute(0x0B04, 0x050B, [:], shortDelay)	//ELECTRICAL_MEASUREMENT Active Power  //Fan does not support power/energy reports
...
def setParameter(paramNum=0, value=null, size=null, delay=shortDelay) {
    ...
    if (paramNum>0 && value!=null) setAttribute(0xfc31,paramNum,size,value, [:], delay)	//no value means we only want to GET the value
...
def getParameter(paramNum=0, delay=shortDelay) {
	...
	if (paramNum<0) {	//special case, if negative then read all params from 0-max (for debugging)
		for(int i = 0;i<=(userSettableParams()+readOnlyParams()).max();i++) {
			getAttribute(0xfc31, i, [:], 100)	//override shortDelay with very short (100ms) delay
		}	
	} else {	//otherwise, just get the requested parameter
		getAttribute(0xfc31, paramNum, [:], delay)
	}
}
