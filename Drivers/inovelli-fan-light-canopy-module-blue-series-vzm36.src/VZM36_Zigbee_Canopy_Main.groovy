...
* 2024-01-09(MA) fix remoteControl (Github Issue #27)
...
* 2023-12-14(MA) Initial BETA release ***VERY ROUGH*** this is a copy of the VZM31 driver, modified to be the VZM36 canopy parent
*
*/
...
def userSettableParams() {   //controls which options are available depending on whether the device is configured as a switch or a dimmer.
    return [95,96,97,98]  //dimmer mode
}
...
def off(childDevice) {
    if (infoEnable) log.info "${device.displayName} off($childDevice)"
    state.lastCommandSent =                        "off($childDevice)"
    state.lastCommandTime = nowFormatted()
    def cmds = []
    //cmds += zigbee.off(shortDelay)
    //cmds = sendHubCommand(new hubitat.device.HubAction("he cmd 0x${device.deviceNetworkId} 0x${getChildId(childDevice)} 0x0006 0x0 {}", hubitat.device.Protocol.ZIGBEE))
    cmds += "he cmd 0x${device.deviceNetworkId} 0xFF 0x0006 0x0 {}"
    if (debugEnable) log.debug "${device.displayName} off $cmds"
    //return sendHubCommand(new hubitat.device.HubMultiAction(delayBetween(cmds, shortDelay), hubitat.device.Protocol.ZIGBEE))
	return cmds
}

def on(childDevice) {
    if (infoEnable) log.info "${device.displayName} on($childDevice)"
    ...
    //cmds += sendHubCommand(new hubitat.device.HubAction("he cmd 0x${device.deviceNetworkId} 0x${getChildId(childDevice)} 0x0006 0x1 {}", hubitat.device.Protocol.ZIGBEE))
...
def traceCluster(String description) {
	...
	log.trace "${device.displayName} ${darkOrange(traceClusterName)} " +
        ...
        (descMap.endpoint==null?" ep:${descMap.sourceEndpoint}":" ep:${descMap.endpoint}") +
...
def parse(String description) {
    Map descMap = zigbee.parseDescriptionAsMap(description)
    if (traceEnable||debugEnable) log.trace "${device.displayName} parse($descMap)"
    try {
		if ((traceEnable||debugEnable) && (zigbee.getEvent(description)!=[:])) log.trace "${device.displayName} zigbee.getEvent ${zigbee.getEvent(description)}"
	} catch (e) {
    ...
    def childId = "${device.id}-${endpoint}"
    def currentChild = getChildDevices()?.find {it.deviceNetworkId == childId}
	currentChild = currentChild?:device
    //currentChild.parse(description)
    //return 
    ...
    switch (clusterInt){
        case 0x0000:    //BASIC CLUSTER
            ...
            switch (attrInt) {
                case 0x0000:
                    if (infoEnable) log.info "${device.displayName}:$endpoint ZCL Version=$valueStr"
        ...
        case 0x0004:    //GROUP CLUSTER
            ...
            switch (attrInt) {
                case 0x0000:
                    if (infoEnable) log.info "${currentChild.displayName} Group Name Support=$valueStr"
                    break
                default:
        ...
        case 0x0005:    //SCENES CLUSTER
            ...
            switch (attrInt) {
                case 0x0000:
                    if (infoEnable) log.info "${currentChild.displayName} Scene Count=$valueStr"
                    break
                case 0x0001:
                    if (infoEnable) log.info "${currentChild.displayName} Current Scene=$valueStr"
                    break
                case 0x0002:
                    if (infoEnable) log.info "${currentChild.displayName} Current Group=$valueStr"
                    break
                case 0x0003:
                    if (infoEnable) log.info "${currentChild.displayName} Scene Valid=$valueStr"
                    break
                case 0x0004:
                    if (infoEnable) log.info "${currentChild.displayName} Scene Name Support=$valueStr"
                    break
                default:
                    log.warn "${device.displayName} " + fireBrick("${clusterName} Unknown Attribute:$attrInt ") //+ descMap
                    break
            ...
            switch (attrInt) {
                case 0x0000:
                    if (descMap.command == "01" || descMap.command == "0A" || descMap.command == "0B"){
                        ...
                        if (infoEnable) log.info "${device.displayName}:$endpoint Switch=$valueInt ($valueStr)"
                        currentChild.sendEvent(name:"switch", value: valueStr)
						def currentLevel = currentChild.currentValue("level")==null?0:currentChild.currentValue("level").toInteger()
                        if (state.model?.substring(0,5)=="VZM35") { //FOR FAN ONLY 
							if (currentChild.currentValue("smartFan")=="Enabled") {
                            ...
                            if (traceEnable||debugEnable) log.trace "${device.displayName}:$endpoint Speed=${newSpeed}"
                            currentChild.sendEvent(name:"speed", value: "${newSpeed}")   
                        }
                ...
                default:
                    if (descMap.profileId=="0000" && descMap.command=="00" && descMap.direction=="00") {
                        ...
                        if (currentChild.settings?.parameter51) {    //not sure why the firmware sends these when there are no bindings
                            if (descMap.data[0]=="00" && infoEnable) log.info "${currentChild.displayName} Bind Command Sent: Switch OFF"
                            if (descMap.data[0]=="01" && infoEnable) log.info "${currentChild.displayName} Bind Command Sent: Switch ON"
                            if (descMap.data[0]=="02" && infoEnable) log.info "${currentChild.displayName} Bind Command Sent: Toggle"
                        }
                    } else log.warn "${device.displayName} " + fireBrick("${clusterName} Unknown Attribute:$attrInt ") //+ descMap
                    break
            }
            ...
            switch (attrInt) {
                case 0x0000:
                    if (descMap.command == "01" || descMap.command == "0A" || descMap.command == "0B"){
                        ...
                        if (infoEnable) log.info "${device.displayName}:$endpoint Level=$valueInt ($valueStr)"
                        currentChild.sendEvent(name:"level", value: percentValue, unit: "%")
		                def newSpeed =""
                        if (state.model?.substring(0,5)=="VZM35") { //FOR FAN ONLY
                            ...
                            if (infoEnable) log.info "${device.displayName}:$endpoint Speed=${newSpeed}"
                            currentChild.sendEvent(name:"speed", value: "${newSpeed}")   
                        }
                    } else log.warn "${device.displayName} " + fireBrick("${clusterName} Unknown Command:$descMap.command ") //+ descMap
					break
            ...
            if (attrInt == null) {
                ...
                switch (attrInt){
                    ...
                    case 13:    //Default Level (local)
						...
						currentChild.sendEvent(name:"levelPreset", value:convertByteToPercent(valueInt))
                        break
                    case 14:    //Default Level (remote)
                    ...
                    case 51:    //Device Bind Number
                        infoMsg += " (Bindings)"
                        currentChild.sendEvent(name:"numberOfBindings", value:valueInt)
                        break
                    case 52:    //Smart Bulb/Fan Mode
                        if (state.model?.substring(0,5)=="VZM35") {
                            infoMsg += " (SFM " + (valueInt==0?red("disabled)"):limeGreen("enabled)"))
                            currentChild.sendEvent(name:"smartFan", value:valueInt==0?"Disabled":"Enabled")
						} else {
                            infoMsg += " (SBM " + (valueInt==0?red("disabled)"):limeGreen("enabled)")) + ")"
                            currentChild.sendEvent(name:"smartBulb", value:valueInt==0?"Disabled":"Enabled")
						}
                        break
				...
				if ((state.model?.substring(0,5)!="VZM35"||state.model?.substring(0,5)!="VZM36") && (attrInt==21 || attrInt==22 || attrInt==158 || attrInt==258)) {  //fan does not support leading/trailing edge dimming
...
def presetLevel(value,childDevice) {
    ...
    cmds += childDevice.setParameter(13, scaledValue)
    cmds += childDevice.setParameter(14, scaledValue)
...
def refresh(option=null,childDevice=null) {
	...
	configParams.each {	//loop through all parameters
		...
		//if (i==23 && (state.model?.substring(0,5)!="VZM35")) quickStartVariables()  //this is noww done in firmware and no longer emulated in the driver
...
def remoteControl(option) {
	...
	sendHubCommand(new hubitat.device.HubMultiAction(delayBetween(cmds, shortDelay), hubitat.device.Protocol.ZIGBEE))
}
...
def setAttribute(Integer cluster, Integer attrInt, Integer dataType, Integer value, Map additionalParams = [:], Integer delay=shortDelay) {
    ...
    zigbee.writeAttribute(cluster, attrInt, dataType, value, additionalParams, delay)
...
def getAttribute(Integer cluster, Integer attrInt=0, Map additionalParams = [:], Integer delay=shortDelay) {
    ...
    //}
	//def cmds = []
    zigbee.readAttribute(cluster, attrInt, additionalParams, delay)
