...
metadata {
    definition (name: "Inovelli VZM36-SN Zigbee Canopy Light", namespace: "InovelliUSA", author: "Sven") { 
...
def BindTarget() {
    if (infoEnable) log.info "${device.label?device.label:device.name}: Target()"
    state.lastCommand = "BindTarget"
    zigbee.command(0x0003, 0x00, [:], 0, "20 00")
}
